...
@ItemReader
public class JdbcCursorItemReaderAdaptor<T> {
		
		@BatchProperty(name="reader.spring.id")
		String springId;
		
		private JdbcCursorItemReader<T> springItemReader;
		...
		public void openFile(CheckpointData cpd) throws Exception {
			 ...
			 springItemReader = (JdbcCursorItemReader<T>) beanFactoryRef.getFactory().getBean(springId);
